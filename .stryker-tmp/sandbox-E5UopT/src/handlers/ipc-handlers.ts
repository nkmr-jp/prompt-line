...
class IPCHandlers {
  ...
  constructor(windowManager: WindowManager, historyManager: IHistoryManager, draftManager: DraftManager, directoryManager: DirectoryManager, settingsManager: SettingsManager) {
    if (stryMutAct_9fa48("640")) {
      ...
      if (stryMutAct_9fa48("642") ? false : stryMutAct_9fa48("641") ? true : (stryCov_9fa48("641", "642"), settings.mdSearch)) {
        if (stryMutAct_9fa48("643")) {
          {}
        } else {
          stryCov_9fa48("643");
          mdSearchLoader.updateConfig(settings.mdSearch);
          logger.info(stryMutAct_9fa48("644") ? "" : (stryCov_9fa48("644"), 'MdSearch config updated from settings'));
        }
      }

      // Instantiate specialized handler classes
      this.pasteHandler = new PasteHandler(windowManager, historyManager, draftManager, directoryManager);
      this.historyDraftHandler = new HistoryDraftHandler(historyManager, draftManager, directoryManager);
  ...
  private setupHandlers(): void {
    if (stryMutAct_9fa48("645")) {
      ...
      logger.info(stryMutAct_9fa48("646") ? "" : (stryCov_9fa48("646"), 'All IPC handlers set up successfully via coordinator'));
    }
  }
  ...
  removeAllHandlers(): void {
    if (stryMutAct_9fa48("647")) {
      ...
      logger.info(stryMutAct_9fa48("648") ? "" : (stryCov_9fa48("648"), 'All IPC handlers removed via coordinator'));
    }
  }

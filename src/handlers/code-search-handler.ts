  ...
  private filterSymbolsByQuery(
    ...
    if (query) {
      ...
      filtered.sort((a, b) => {
        ...
        // Add mtime bonus if available (capped at 500)
        const aMtimeBonus = a.mtimeMs ? Math.min(calculateFileMtimeBonus(a.mtimeMs), 500) : 0;
        const bMtimeBonus = b.mtimeMs ? Math.min(calculateFileMtimeBonus(b.mtimeMs), 500) : 0;

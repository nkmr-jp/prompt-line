...
class SettingsManager {
  ...
  private addCommentsToSettings(settings: UserSettings): string {
    // Helper to format arrays for YAML output
    const formatArrayOrEmpty = (arr: unknown[] | undefined): string => {
      if (!arr || arr.length === 0) return '[]';
      return arr.map(item => `\n    - "${item}"`).join('');
    };

    // Helper to format extensions object for YAML output
    const formatExtensionsOrEmpty = (ext: Record<string, string> | undefined): string => {
...
fileSearch:
  ...
  excludePatterns: ${formatArrayOrEmpty(settings.fileSearch?.excludePatterns)}  # Additional exclude patterns (e.g., ["*.log", "*.tmp"])
  includePatterns: ${formatArrayOrEmpty(settings.fileSearch?.includePatterns)}  # Force include patterns (override .gitignore)
...
fileOpener:
  ...
  # Example: { ts: "WebStorm", md: "Typora", pdf: "Preview" }
  extensions: ${formatExtensionsOrEmpty(settings.fileOpener?.extensions)}
...
# Leave as empty array [] if not using this feature
#

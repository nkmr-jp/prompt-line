...
import { addCommentsToSettings } from './settings-comment-generator';
...
class SettingsManager {
  ...
  private async loadSettings(): Promise<void> {
    try {
      ...
      await this.readAndParseSettings();
    } catch (error) {
  ...
  private async readAndParseSettings(): Promise<void> {
    try {
      const data = await fs.readFile(this.settingsFile, 'utf8');
      await this.parseAndApplySettings(data);
    } catch (error) {
      await this.handleSettingsReadError(error);
    }
  }

  private async parseAndApplySettings(data: string): Promise<void> {
    ...
    if (parsed && typeof parsed === 'object') {
      ...
      logger.debug('Settings loaded from YAML file', { file: this.settingsFile });
    } else {
  ...
  private async handleSettingsReadError(error: unknown): Promise<void> {
  ...
  async saveSettings(): Promise<void> {
    try {
      const settingsWithComments = addCommentsToSettings(this.currentSettings);

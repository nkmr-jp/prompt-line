...
入力フィールドで `/` を入力することで、カスタムスラッシュコマンドにすばやくアクセスできます。

### コマンドディレクトリの設定

`~/.prompt-line/settings.yml` に `commands.directories` 設定を追加します：

```yaml
# カスタムスラッシュコマンドの設定
commands:
  # カスタムスラッシュコマンドの.mdファイルを含むディレクトリ
  # 重複するコマンド名がある場合、最初のディレクトリが優先されます
  directories:
...
各コマンドはYAMLフロントマターを持つMarkdownファイル（`.md`）です：

```markdown
---
description: このコマンドの簡単な説明
argument-hint: <Tab選択後に表示されるオプションの引数ヒント>
---

コマンドプロンプトのテンプレートをここに記述。
$ARGUMENTS を使用してユーザー指定の引数を含めることができます。
```
...
以下のコードをレビューしてフィードバックをお願いします：
$ARGUMENTS
```

### コマンドの使い方

1. 入力フィールドで `/` を入力して利用可能なコマンドを表示
2. 矢印キーまたは `Ctrl+j`/`Ctrl+k` でナビゲート
3. **Enter** を押してすぐに選択して貼り付け
4. **Tab** を押して選択し、引数を編集（argument-hintが設定されていれば表示）
5. **Escape** を押してサジェストを閉じる

### フロントマターのフィールド

| フィールド | 必須 | 説明 |
|-----------|------|------|
| `description` | いいえ | サジェストリストに表示される簡単な説明 |
| `argument-hint` | いいえ | Tab選択後に表示されるヒントテキスト（設定されていない場合はdescriptionが使用される） |

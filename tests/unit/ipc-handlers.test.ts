...
describe('IPCHandlers', () => {
    ...
    describe('removeAllHandlers', () => {
        test('should remove all registered handlers', () => {
            ...
            // Should be called for each handler (count: 45 handlers)
            // paste-handler: 2, window-handler: 3, history-draft-handler: 14 (includes 2 at-path cache handlers + save-draft-to-history)
            // system-handler: 5, file-handler: 4 (includes reveal-in-finder), custom-search-handler: 11 (includes has-command-file + 3 slash command cache handlers + invalidate-custom-search), code-search-handler: 1
            // usage-history-handler: 6 (record/get bonuses for file, symbol, agent)
            expect(ipcMain.removeAllListeners).toHaveBeenCalledTimes(45);

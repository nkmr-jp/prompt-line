...
**The 200ms budget is a deadline for the whole call, not a per-`read` timeout.** `SO_RCVTIMEO` alone cannot bound this: it restarts on every byte received, so a peer dribbling one byte at a time holds the call open indefinitely (measured: 24s against a fake server sending 1 byte per 150ms; the only ceiling was the caller's 3s `SIGTERM`). The read loop therefore re-derives the remaining time before each `read` and gives up when it is spent.

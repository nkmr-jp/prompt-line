...
Orca (`com.stablyai.orca`) can have many agent terminals producing output in parallel, so tty mtime cannot reliably identify the focused terminal. The detector calls Orca's bundled `orca terminal show --json` to resolve the terminal selected in the UI, then limits process-tree CWD detection to that terminal's `worktreePath`. If the CLI is unavailable or does not respond within 500ms, it falls back to the generic process-tree detector.
